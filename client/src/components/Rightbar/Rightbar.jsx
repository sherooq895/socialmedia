import React from 'react'
import './Rightbar.css'

function Rightbar() {
    return (
        <div className='p-4 m-3 bg- bg-white rounded-lg'>
            <div className='text-[#153f7c] text-xl mb-5'>Suggetions For You</div>


            <div className='scoller-right-bar  '>
                <div className='flex mb-3'>
                    <div className='scoller-item'>
                        <img src="http://t1.gstatic.com/licensed-image?q=tbn:ANd9GcQr_2BReD2Ca6h7cPtwjM46Tyqx5MJDGV-uvAAR1009Gcg9LzG6KS4-bBw4xiAYPWVTfBOtEk2E5-6LidY" alt="" />
                    </div>
                    <div>
                    <div className='mt-2 ml-2 text-lg text-[#153f7c]'>sdsfff</div>
                    <div><button className='bg-[#153f7c] hover:bg-yellow-600 text-white font-bold py-1 px-8 ml-2 rounded'>Follow</button></div>

                    </div>
                    
                </div>
                <div className='flex mb-3'>
                    <div className='scoller-item'>
                        <img src="http://t1.gstatic.com/licensed-image?q=tbn:ANd9GcQr_2BReD2Ca6h7cPtwjM46Tyqx5MJDGV-uvAAR1009Gcg9LzG6KS4-bBw4xiAYPWVTfBOtEk2E5-6LidY" alt="" />
                    </div>
                    <div>
                    <div className='mt-2 ml-2 text-lg text-[#153f7c]'>sdsfff</div>
                    <div><button className='bg-[#153f7c] hover:bg-[#0a1b36] text-white font-bold py-1 px-8 ml-2 rounded'>Follow</button></div>

                    </div>
                    
                </div>
                <div className='flex mb-3'>
                    <div className='scoller-item'>
                        <img src="http://t1.gstatic.com/licensed-image?q=tbn:ANd9GcQr_2BReD2Ca6h7cPtwjM46Tyqx5MJDGV-uvAAR1009Gcg9LzG6KS4-bBw4xiAYPWVTfBOtEk2E5-6LidY" alt="" />
                    </div>
                    <div>
                    <div className='mt-2 ml-2 text-lg text-[#153f7c]'>sdsfff</div>
                    <div><button className='bg-[#153f7c] hover:bg-[#0a1b36] text-white font-bold py-1 px-8 ml-2 rounded'>Follow</button></div>

                    </div>
                    
                </div>
                <div className='flex mb-3'>
                    <div className='scoller-item'>
                        <img src="http://t1.gstatic.com/licensed-image?q=tbn:ANd9GcQr_2BReD2Ca6h7cPtwjM46Tyqx5MJDGV-uvAAR1009Gcg9LzG6KS4-bBw4xiAYPWVTfBOtEk2E5-6LidY" alt="" />
                    </div>
                    <div>
                    <div className='mt-2 ml-2 text-lg text-[#153f7c]'>sdsfff</div>
                    <div><button className='bg-[#153f7c] hover:bg-[#0a1b36] text-white font-bold py-1 px-8 ml-2 rounded'>Follow</button></div>

                    </div>
                    
                </div>
                <div className='flex mb-3'>
                    <div className='scoller-item'>
                        <img src="http://t1.gstatic.com/licensed-image?q=tbn:ANd9GcQr_2BReD2Ca6h7cPtwjM46Tyqx5MJDGV-uvAAR1009Gcg9LzG6KS4-bBw4xiAYPWVTfBOtEk2E5-6LidY" alt="" />
                    </div>
                    <div>
                    <div className='mt-2 ml-2 text-lg text-[#153f7c]'>sdsfff</div>
                    <div><button className='bg-[#153f7c] hover:bg-[#0a1b36] text-white font-bold py-1 px-8 ml-2 rounded'>Follow</button></div>

                    </div>
                    
                </div>
            </div>
        </div>
    )
}

export default Rightbar